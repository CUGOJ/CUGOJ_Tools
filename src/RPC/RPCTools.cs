...
public static class RPCTools
{
    ...
    public static Base NewBase()
    {
        var res = new Base();
        res.Extra = new();
        var serviceID = Context.Context.ServiceBaseInfo.ServiceID;
        if (CUGOJ.CUGOJ_Tools.Tools.CommonTools.IsEmptyString(serviceID))
            serviceID = "unknown";
        var activity = System.Diagnostics.Activity.Current;
        if (activity != null)
        {
            res.Extra["TraceContext"] = activity.Id;
            res.Extra["ServiceID"] = serviceID;
            res.Extra["UserID"] = Context.Context.UserID;
        }
        else
        {
            using var rootActivity = new Activity(RPCService.ServiceName);
            rootActivity.Start();
            res.Extra["TraceContext"] = rootActivity.Id;
            res.Extra["ServiceID"] = serviceID;
            res.Extra["UserID"] = Context.Context.UserID;
        }
        return res;
